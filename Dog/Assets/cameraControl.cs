...
public class cameraControl : MonoBehaviour
{
	...
	void Update()
	{
		if(focusObj != null){
			if(splitFocusObj == null){
				...
				offsetCamera = new Vector3(0,splitDistance + 8, -splitDistance - 8);
				focusPos = focusObj.transform.position + (splitFocusObj.transform.position - focusObj.transform.position) / 1.25f;
			}
		}
		
		targetPos = focusPos + offsetCamera + (zoomOffset * zoomEffect) + touchOffset;

		mCam.transform.position = Vector3.SmoothDamp(mCam.transform.position, targetPos, ref velocity , 30 * Time.unscaledDeltaTime);
	...
	public void buildMode(){
		...
		offsetCamera = new Vector3(1.6f,40,-42);
