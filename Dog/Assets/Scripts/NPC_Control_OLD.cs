...
public class NPC_Control_OLD : MonoBehaviour
{
  ...
  private int TempMobSpawnNo;
    ...
    public void DespawnMob (GameObject MobToDespawn){
      ...
      for (int i = 0; i<NPCPrefabIndex.Length; i++){
        if(NPCPrefabIndex[i].GetComponent<NPC>().MobCodeNo == MobToDespawn.GetComponent<NPC>().MobCodeNo){
          NPCPrefabRollCall[i] = false;
